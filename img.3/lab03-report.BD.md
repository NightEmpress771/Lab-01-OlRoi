...
-- ##########################################################################
--
-- **Виконав:** [Бугайчук Денис]
-- **Група:** [ІПЗ-32]
-- **Дата виконання:** [3 листопада 2025]
-- **Варіант:** Система управління бібліотекою
--
-- ##########################################################################
-- ## Мета роботи
--
-- Здобути практичні навички безпечної модифікації даних у реляційних базах даних,
-- опанувати механізми транзакцій для забезпечення цілісності даних,
-- навчитися працювати з обмеженнями цілісності та каскадними операціями
-- при зміні взаємопов'язаних записів, використовуючи сценарії бібліотечної системи.
--
-- ##########################################################################
-- ## Виконання роботи
--
...
-- #### Крок 1. Створення структури бази даних з обмеженнями цілісності
-- Створено базові таблиці (`authors`, `readers`, `books`, `loans`) з основними обмеженнями
-- (PRIMARY KEY, UNIQUE, CHECK та FOREIGN KEY з ON DELETE RESTRICT), що забезпечує
-- запобігання видаленню авторів, якщо на них є посилання з таблиці книг.
--
-- ```sql
...
CREATE TABLE books (
    ...
    description TEXT,
    CONSTRAINT chk_available_less_than_total CHECK (copies_available <= copies_total)
);
...
-- ```
-- [![Скріншот 1](image.png)СКРІНШОТ 1: Результат виконання DDL скрипту, що демонструє створення таблиць та обмежень]

-- #### Крок 2. Додавання тестових даних
-- Виконано вставку тестового набору даних для перевірки обмежень та функціоналу.
--
-- ```sql
...
('Тіні забутих предків', 4, '978-1-00000-000-4', 1911, 4, 4), 
('Intermezzo', 4, '978-1-00000-000-5', 1908, 2, 2),
('Земля', 5, '978-1-00000-000-6', 1901, 3, 3), 
('Contra spem spero!', 2, '978-1-00000-000-7', 1890, 1, 1);
COMMIT;
-- ```
-- [![Скріншот 2](image-1.png)СКРІНШОТ 2: Вибірка даних з таблиць authors та books після вставки]

-- #### Крок 3. Транзакція видачі книги (fn_checkout_book)
-- Реалізовано функцію видачі книги як атомарну транзакцію, що включає
-- консультативне блокування (pg_advisory_xact_lock) для уникнення race conditions
-- при зміні кількості доступних копій.
--
-- ```sql
...
DECLARE
    v_available INTEGER;
BEGIN
    -- Використання консультативного блокування для запобігання гонки (race conditions)
    ...
    -- Атомарна зміна: зменшення копій та створення запису
...
-- ```
-- [![Скріншот 2](image-2.png)СКРІНШОТ 3: Результат виконання fn_checkout_book(1, 2) та оновлені дані в loans і books]

-- #### Крок 4. Демонстрація часткового відкату транзакції (SAVEPOINT)
-- Продемонстровано використання точки збереження для відкату помилкової внутрішньої операції,
-- зберігаючи при цьому успішні зміни, що відбулися до SAVEPOINT.
--
-- ```sql
...
BEGIN;
    ...
    -- Зміна 1 (буде збережена)
    UPDATE readers SET full_name = 'Петренко Іван Миколайович' WHERE reader_id = 1;

    SAVEPOINT nested;
    -- Зміна 2 (буде скасована)
    UPDATE books SET title = 'Кобзар (Ювілейне видання)' WHERE book_id = 1;
    -- Відкат до точки після першої зміни
    ROLLBACK TO SAVEPOINT nested;

    -- Commit only the first change (reader update)
COMMIT;
-- ```
-- [![Скріншот 4](image-3.png)СКРІНШОТ 4: Демонстрація транзакції з SAVEPOINT та результат SELECT із таблиці readers (змінено) та books (не змінено)]

-- ##########################################################################
-- ### Рівень 2. Розширена функціональність
-- ##########################################################################

-- #### Крок 1. Створення додаткових таблиць для аудиту та версіонування
-- Додано таблиці для розширеної логіки системи: `reservations`, `change_history`, `book_versions` та `deferred_tasks`.
--
-- ```sql
-- Reservations table
...
-- #### Крок 2. Тригери для аудиту та версіонування
-- Реалізовано тригерну функцію `trg_log_changes` для запису всіх модифікацій до `change_history`
-- та тригерну функцію `trg_book_versioning` для збереження старої версії книги перед оновленням.
--
-- ```sql
...
CREATE TRIGGER trg_authors_changes AFTER INSERT OR UPDATE OR DELETE ON authors FOR EACH ROW EXECUTE FUNCTION trg_log_changes();
CREATE TRIGGER trg_books_changes AFTER INSERT OR UPDATE OR DELETE ON books FOR EACH ROW EXECUTE FUNCTION trg_log_changes();
-- (інші тригери)
...
-- ```
-- [![Скріншот 5](image-4.png)СКРІНШОТ 5: Приклад запису аудиту в таблиці change_history]

-- #### Крок 3. Складна транзакція повернення книги (fn_return_book)
-- Реалізовано функцію повернення, яка атомарно оновлює статус видачі та збільшує
-- кількість доступних копій у таблиці `books`.
--
-- ```sql
-- Return a book
...
-- Тестування
SELECT * FROM fn_return_book(1, 2); -- Повернення книги
-- ```
-- [![Скріншот 6](image-5.png)СКРІНШОТ 6: Результат виконання fn_return_book та відповідні записи в loans (оновлено) і books (збільшено copies_available)]

-- ##########################################################################
-- ### Рівень 3. Творче розширення
-- ##########################################################################

-- #### Крок 1. Система відновлення версій (fn_revert_book_to_version)
-- Створено функцію для відкату стану книги до будь-якої попередньої версії,
-- зафіксованої тригером версіонування.
--
-- ```sql
...
-- #### Крок 2. Відкладені та автоматизовані операції
-- Створено таблицю `deferred_tasks` та функцію `fn_process_deferred` для імітації
-- фонової обробки, наприклад, скасування прострочених бронювань.
--
-- ```sql
...
-- ```
-- [![Скріншот 7](image-6.png)СКРІНШОТ 7: Результат запиту до представлення vw_recent_changes, що показує останні зміни]
...
-- ##########################################################################
--
-- В ході лабораторної роботи було успішно розроблено та реалізовано комплексний
-- SQL-скрипт для системи управління бібліотекою, що охоплює:
--
-- 1.  **Цілісність даних:** Створено таблиці з жорсткими обмеженнями (FOREIGN KEY ON DELETE RESTRICT),
--     які запобігають небезпечним операціям (наприклад, видаленню автора, на якого є посилання).
-- 2.  **Атомарність операцій (Транзакції):** Розроблено функції `fn_checkout_book` та `fn_return_book`,
--     які використовують транзакції та блокування (FOR UPDATE, pg_advisory_xact_lock) для
--     забезпечення консистентності залишків книг (`copies_available`) під час видачі та повернення.
-- 3.  **Гнучкість:** Продемонстровано використання `SAVEPOINT` для часткового відкату транзакцій,
--     що дозволяє обробляти внутрішні помилки без втрати попередньо успішних змін.
-- 4.  **Аудит та Версіонування:** Впроваджено тригерний механізм для логування всіх модифікацій
--     (`change_history`) та версіонування критичних даних (`book_versions`), що забезпечує
--     відстеження історії змін та можливість відкату стану за допомогою `fn_revert_book_to_version`.
-- 5.  **Автоматизація:** Створено механізм відкладених завдань для обробки фонових процесів.
--
-- Отримані навички дозволяють створювати надійні та стійкі до помилок додатки,
-- де цілісність бізнес-логіки гарантується на рівні бази даних.
--

-- ##########################################################################
-- ## Додаток А: Всі скріншоти розміщено в папці img.3
-- ##########################################################################
-- ## Додаток Б: Повний код
